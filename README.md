...
게임을 시작하면 바로 떨어지지 않습니다. 공들이 시작 구역에서 **배치 패턴(일렬·지그재그·타원·V자·X자·산개)** 사이를 계속 옮겨다니고, 전원이 그걸 지켜보다가 **방장이 "🎲 지금 떨어뜨리기!"를 누르는 순간의 위치에서** 낙하가 시작됩니다 (45초 내 미입력 시 자동 낙하). 새 배치 패턴은 `server/game.js`의 `SPAWN_PATTERNS`에 함수 하나만 추가하면 됩니다.
...
- `TIME_SCALE` — **낙하 배속** (기본 10). 낙하가 시작되면 물리 시뮬레이션을 틱당 이 배수만큼 돌려 공이 그만큼 빠르게 떨어집니다. 셔플 단계는 항상 실시간(1배속)이고, 아이템 지속시간·폭탄 재생성 등 게임 내 타이머는 게임 시간 기준이라 배속을 바꿔도 밸런스가 유지됩니다
