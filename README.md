...
- **🎲 셔플 낙하** (기본): 공들이 배치 패턴 사이를 옮겨다니다 방장이 누르는 순간 일괄 낙하
- **✂️ 줄 자르기**: 공들이 **꼬인 줄에 매달린 채로 시작**합니다. 참가자가 돌아가며 위쪽 번호 손잡이를 하나씩
  잘라내는데, 줄이 서로 여러 번 교차해서 **어떤 줄이 누구의 공인지 눈으로 따라가기 어렵습니다.**
  자른 줄의 공만 즉시 낙하 (이 단계는 실시간 진행) — 마지막 줄이 잘리면 배속 낙하로 전환됩니다.
  차례에서 12초를 넘기면 무작위 줄이 자동으로 잘립니다. 아이템은 절단 단계에서도 사용 가능!
...
- `TIME_SCALE` — **낙하 배속(공의 속도)**, `server/config.js` 설정 파일에서 조절 (기본 5). 환경변수 `TIME_SCALE=3 npm start` 또는 `scripts\start.bat`의 `set TIME_SCALE=` 로도 지정할 수 있습니다. 낙하가 시작되면 물리 시뮬레이션을 틱당 이 배수만큼 돌려 공이 그만큼 빠르게 떨어집니다. 셔플·줄 자르기 단계는 항상 실시간(1배속)이고, 아이템 지속시간·폭탄 재생성 등 게임 내 타이머는 게임 시간 기준이라 배속을 바꿔도 밸런스가 유지됩니다
